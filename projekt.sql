...
PACKAGE BODY PACKAGE_USERACTIONS AS
  ...
  BEGIN
    if recordType = 1 then
        ...
        open recordTypeCur for select * from lap where rownum <= recordLimit order by minute asc, second asc, milisecond asc;
    end if;
    loop
        fetch recordTypCur into results;
            DBMS_OUTPUT.PUT_LINE('ID okrazena' || wynik.id);
        exit when recordTypCur%notfound;
    end loop;
  END getRecords;
